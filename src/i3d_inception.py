...
# Adapted from https://github.com/dlpbc/keras-kinetics-i3d/blob/master/i3d_inception.py
# Only the model definition is included here for brevity and clarity.
...
WEIGHTS_PATH_NO_TOP = {
    ...
    'flow_imagenet_and_kinetics' : 'https://github.com/dlpbc/keras-kinetics-i3d/releases/download/v0.2/flow_inception_i3d_imagenet_and_kinetics_no_top.h5'
}
...
def Inception_Inflated3d(include_top=True,
    ...
    (Full model code from keras-kinetics-i3d)
    """
    import warnings
    import numpy as np
    from keras.models import Model
    from keras import layers
    from keras.layers import Activation, Dense, Input, BatchNormalization, Conv3D, MaxPooling3D, AveragePooling3D, Dropout, Reshape, Lambda, GlobalAveragePooling3D
    from tensorflow.keras.utils import get_file
    from keras import backend as K
    ...
    if weights in WEIGHTS_NAME:
        ...
        if K.backend() == 'theano':
            pass  # layer_utils.convert_all_kernels_in_model(model)  # Not needed in TF
    ...
    return model
    # --- END MODEL BUILDING LOGIC ---

# Usage:
# model = Inception_Inflated3d(input_shape=(64, 224, 224, 3), classes=825, weights='models/rgb_inception_i3d_kinetics_only_tf_dim_ordering_tf_kernels_no_top.h5', include_top=False) 